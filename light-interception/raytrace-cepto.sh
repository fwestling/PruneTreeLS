...
go $1 $options_row $options_tree $options_from $options_repeats $options_output_dir $options_cepto_file $options_openair_file $options_coeff $options_voxel_size $options_voxel_weight $options_rotate $options_stretch $options_time_steps $options_geo_rot $options_result_policy $options_result_radius $options_raytrace_scale
			csv-shuffle --fields=dx,dy,dz,cid,meas,opar,sim,sim_adj,dff -o=dx,dy,dz,cid,dff > $dataDir/optimal-dx.csv
		cat $dataDir/allMatches.csv | csv-eval --fields=,,,x,y,z,,,,,,,,x2,y2,z2,,,, "x=x-x2;y=y-y2;z=z-z2" |
			csv-shuffle --fields=,,,dx,dy,dz,,cid,nrg,opar,,,,,,,,,,nrg2 -o=dx,dy,dz,cid,nrg,opar,nrg2 | tee $dataDir/allDxyz.csv |
			csv-eval --fields=dx,dy,dz,cid,meas,opar,sim "sim_adj=sim*$ratio ; dff=abs(sim_adj-meas)" |
			csv-sort --fields=,,,id,,,,,d > $dataDir/sortedDxyz.csv
	elif [[ $policy == "opti-random" ]]
	then
		## Optimal value
		okj=$(cat $outDir/open.$tTime.nrg.csv)
		ratio="opar/($voxel_size*$voxel_size*$okj)"
		[[ $no_open ]] && ratio="1.72/($voxel_size*$voxel_size)"

		stats=$(cat $outDir/nrg-gnd.$coeff.$tTime.csv | csv-calc mean,stddev --fields=,,,,,,,,,x --format=3ui,3d,3ui,d)
		dist_mean=$(echo "$stats" | cut -d, -f1)
		dist_stddev=$(echo "$stats" | cut -d, -f2)

		cat $cepto_file |
			sed -r 's/(^[^,]*,[^,]*,[^,]*),[0-9.-]*,([0-9.-]*),([0-9.-]*),([0-9.-]*),([0-9.-]*),([0-9.-]*),([0-9.-]*),([0-9.-]*),([0-9.-]*)/\1,1,\2\n\1,2,\3\n\1,3,\4\n\1,4,\5\n\1,5,\6\n\1,6,\7\n\1,7,\8\n\1,8,\9/' |
			csv-eval --format=t,3ui,d --fields=t,sou,pos,seg,nrg "l = sou*1000 + seg*100 + pos" |
			csv-shuffle --fields=t,sou,pos,seg,nrg,cid -o=t,cid,nrg |
			csv-sort --fields=t | csv-time-join --nearest "$open_file" |
			csv-shuffle --fields=,cid,nrg,,opar -o=cid,nrg,opar |
 			csv-join --fields=id "$dataDir/cpts.$tc.csv;fields=,,,,,,,id" |
			csv-shuffle --fields=cid,nrg,opar,vx,vy,vz,x,y,z,w,cid2 -o=vx,vy,vz,x,y,z,w,cid,nrg,opar |
			points-join --fields=,,,x,y,z "$outDir/nrg-gnd.$coeff.$tTime.csv;fields=,,,x,y,z" --radius $radius --all |
			csv-shuffle --fields=vx,vy,vz,x,y,z,w,cid,nrg,opar,vx2,vy2,vz2,x2,y2,z2,w2,cid2,uid2,nrg2 -o=vx,vy,vz,x,y,z,w,cid,nrg,opar,nrg2 |
			csv-eval --fields=vx,vy,vz,x,y,z,w,cid,meas,opar,sim "meas=random.normal($dist_mean,$dist_stddev) ; sim_adj=sim*$ratio ; dff=abs(sim_adj-meas)" |
			csv-sort --fields=,,,,,,,id,,,,,d  --min |
			csv-paste "-" "value=1" |
			csv-shuffle --fields=vx,vy,vz,x,y,z,w,cid,meas,opar,sim,sim_adj,dff,opn -o=opn,vx,vy,vz,x,y,z,w,cid,sim
	elif [[ $policy == "opti-random-loc" ]]
	then
		## Optimal value
		okj=$(cat $outDir/open.$tTime.nrg.csv)
		ratio="opar/($voxel_size*$voxel_size*$okj)"
		...
		stats=$(cat $outDir/nrg-gnd.$coeff.$tTime.csv | csv-calc mean,stddev --fields=,,,,,,,,,x --format=3ui,3d,3ui,d)
		dispt_mean=$(echo "$stats" | cut -d, -f1)
		dist_stddev=$(echo "$stats" | cut -d, -f2)

		cat $cepto_file |
			csv-eval --fields=,,pos,avg,s1,s2,s3,s4,s5,s6,s7,s8 "ran = (s1+s7)/avg" |
			csv-sort --fields=t,sou,pos,avg,s1,s2,s3,s4,s5,s6,s7,s8,ran --order=sou,ran |
			csv-paste "-" "line-number" | csv-eval --fields=,sou,,,,,,,,,,,,line "line=(line+1)-(49*sou)" |
			csv-shuffle --fields=t,sou,pos,avg,s1,s2,s3,s4,s5,s6,s7,s8,ran,line --output-fields=t,sou,line,avg,s1,s2,s3,s4,s5,s6,s7,s8 |
			sed -r 's/(^[^,]*,[^,]*,[^,]*),[0-9.-]*,([0-9.-]*),([0-9.-]*),([0-9.-]*),([0-9.-]*),([0-9.-]*),([0-9.-]*),([0-9.-]*),([0-9.-]*)/\1,1,\2\n\1,2,\3\n\1,3,\4\n\1,4,\5\n\1,5,\6\n\1,6,\7\n\1,7,\8\n\1,8,\9/' |
			csv-eval --format=t,3ui,d --fields=t,sou,pos,seg,nrg "l = sou*1000 + seg*100 + pos" |
			csv-shuffle --fields=t,sou,pos,seg,nrg,cid -o=t,cid,nrg |
			csv-sort --fields=t | csv-time-join --nearest "$open_file" |
			csv-shuffle --fields=,cid,nrg,,opar -o=cid,nrg,opar |
 			csv-join --fields=id "$dataDir/cpts.$tc.csv;fields=,,,,,,,id" |
			csv-shuffle --fields=cid,nrg,opar,vx,vy,vz,x,y,z,w,cid2 -o=vx,vy,vz,x,y,z,w,cid,nrg,opar |
			points-join --fields=,,,x,y,z "$outDir/nrg-gnd.$coeff.$tTime.csv;fields=,,,x,y,z" --radius $radius --all | tee $dataDir/allMatches.csv |
			csv-shuffle --fields=vx,vy,vz,x,y,z,w,cid,nrg,opar,vx2,vy2,vz2,x2,y2,z2,w2,cid2,uid2,nrg2 -o=vx,vy,vz,x,y,z,w,cid,nrg,opar,nrg2 |
			csv-eval --fields=vx,vy,vz,x,y,z,w,cid,meas,opar,sim "sim_adj=sim*$ratio ; dff=abs(sim_adj-meas)" |
			csv-sort --fields=,,,,,,,id,,,,,d  --min |
			csv-paste "-" "value=1" |
			csv-shuffle --fields=vx,vy,vz,x,y,z,w,cid,meas,opar,sim,sim_adj,dff,opn -o=opn,vx,vy,vz,x,y,z,w,cid,sim
		# cat $dataDir/allMatches.csv | csv-eval --fields=,,,x,y,z,,,,,,,,x2,y2,z2,,,, "x=x-x2;y=y-y2;z=z-z2" |
		...
		# If the policy is not recognised, just use mean.
		echo "Unknown policy $policy; using mean instead" >&2
		cat $dataDir/cpts.$tc.csv | points-join --fields=,,,x,y,z "$outDir/nrg-gnd.$coeff.$tTime.csv;fields=,,,x,y,z" --radius $radius --all |
			csv-calc mean --fields=,,,,,,,id,i,j,k,x,y,z,w,,,nrg --format=3ui,3d,2ui,3ui,3d,3ui,d |
			csv-paste "-" "value=1" |
			csv-shuffle --fields=i,j,k,x,y,z,w,nrg,cid,opn -o=opn,i,j,k,x,y,z,w,cid,nrg
	# rm $dataDir/temp.$tc
	fi |

	# At this point, we should have nrg-pts.tc.time.csv with fields opn,vx,vy,vz,x,y,z,vw,cid,nrg
	#echo -n "$tc, " >&2
	# Extract data for single transmission coefficient
	# Move density here
	csv-sort --fields=,,,,,,,,id -u |
	 csv-shuffle \
	 	--fields=,,,,,,,,cid,nrg, \
	 	-o=cid,nrg | # Select energy of interest, discard the rest
	 csv-eval --fields=l, --format=ui,d "south=floor(l/1000);r=l-south*1000;seg=floor(r/100);id=l-south*1000-seg*100" -o=south,seg,id |
	 csv-sort --fields=,,a,b,c --order=a,c,b | # Sort by north/south, then id, then seg
	 csv-shuffle --fields=uid,nrg,sou,seg,pos -o=sou,pos,nrg,seg | # Clean up data a bit more  ## at this point, we have sou,pos,nrg,seg...
	 sed 's/,[1-7]$/;/' | sed 's/,8$/:/' | tr '\n' ',' | sed 's/;,[01],[0-9]*,/,/g' | tr ':' '\n' | sed 's/^,//' |  # Remove most line breaks to generate standard form
	 csv-eval --fields=,,a,b,c,d,e,f,g,h --format=2ui,8d "avg=(a+b+c+d+e+f+g+h)/8" | # Calculate average
	 csv-shuffle --fields=sou,pos,q,w,e,r,t,y,u,i,avg -o=sou,pos,avg,q,w,e,r,t,y,u,i | # Finalise standard form
	 csv-paste "-" "value=0" |
