...
BOMParser contains methods to retrieve and parse BOM radiation data.

Example:
--------------
date = datetime.datetime.strptime('20160401')
energy = BOMParser().solarRadFromBOM(date,-25,150)
...
class BOMParser(object):
	...
	def plotNCData(self,date):
		...
		for j in range(0,len(latitudes)):
			for i in range(0,len(longitudes)):
				radiation.append(solar_radiance[day_of_month-1][j][i])
	...
	def __NCread(self,nc_file):
		"""
		Reads BOM data from .nc file. Stores parsed data in internal dictionairy if .nc has not been parsed previously.

		return: list[longitudes], list[latitudes],list[solar_radiance]
		"""
		if( not(nc_file in self.BOM_files) ):
			with netCDF4.Dataset(nc_file,'r') as data:
				longitudes = data.variables['longitude'][:]
				latitudes = data.variables['latitude'][:]
				solar_radiance = data.variables['rad_day'][:]
	...
	def __solarRadiationAtLatLongFromNC(self,nc_file,lat,lon,day_of_month):
		...
		return solar_radiance[day_of_month-1][index_lat][index_lon]
	...
	def __reloadFromBOM(self,date):
		...
		"""
		dates = self.__computeTimespan(date)

		if(self.data_folder == ""):
			path = dates + '.nc'
		else:
			path = self.data_folder + '/' + dates + '.nc'
	...
	def __getDataFromBOM(self,date):
		...
		year = date[0:4] 
		urllib.urlretrieve('http://rs-data1-mel.csiro.au/thredds/fileServer/bawap/rad/day/'+ year +'/bom-rad_day-'+dates+'.nc',path)
...
def generate_csv( args ):
	BOM = BOMParser(args.data_folder)
